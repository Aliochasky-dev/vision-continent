...
public class EvenementService {
    ...
    public Page<EvenementDto.PageResponse> getCatalogue(
        ...
        // Valeurs autorisées (alignées avec l'OpenAPI + frontend)
        String sortValue = (sort == null || sort.isBlank()) ? "recent" : sort.trim().toLowerCase();

        Sort sorting = switch (sortValue) {
            case "recent"       -> Sort.by("createdAt").descending();
            case "trending"     -> Sort.by("nbTraders").descending();      // ou volumeTotalFcfa si tu préfères
            case "closing_soon" -> Sort.by("dateFin").ascending();
            default -> throw new VisionException(
                    "Valeur de tri invalide. Valeurs acceptées : recent, trending, closing_soon");
        };
