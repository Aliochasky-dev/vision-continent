...
import com.VISION.continent.dtos.MarcheDto;
...
public class PositionService {
    ...
    public PositionDto.Response placerMise(PositionDto.Request req, Long userId) {
        ...
        // Incrémente nbTraders seulement si c'est la première mise de cet utilisateur sur cet événement
        boolean premierePositionSurEvenement = !positionRepository
                .existsByUserIdAndMarche_EvenementId(userId, marche.getEvenement().getId());

        if (premierePositionSurEvenement) {
            Evenement ev = marche.getEvenement();
            ev.setNbTraders((ev.getNbTraders() == null ? 0 : ev.getNbTraders()) + 1);
        }
        ...
        evenementRepository.save(marche.getEvenement());
    ...
    @Transactional(readOnly = true)
    public MarcheDto.CagnotteResponse getCagnotte(UUID marcheId) {
        Marche marche = marcheRepository.findById(marcheId)
                .orElseThrow(() -> new VisionException("Marché introuvable"));

        List<Position> positions = positionRepository.findByMarcheId(marcheId);

        long nbOui = positions.stream()
                .filter(p -> p.getChoix() == Position.Choix.OUI)
                .map(p -> p.getUser().getId())
                .distinct().count();

        long nbNon = positions.stream()
                .filter(p -> p.getChoix() == Position.Choix.NON)
                .map(p -> p.getUser().getId())
                .distinct().count();

        return MarcheDto.CagnotteResponse.builder()
                .marcheId(marche.getId())
                .question(marche.getQuestion())
                .statut(marche.getStatut())
                .poolOuiFcfa(marche.getPoolOuiFcfa())
                .poolNonFcfa(marche.getPoolNonFcfa())
                .volumeTotalFcfa(marche.getVolumeFcfa())
                .prixOui(marche.getPrixOui())
                .prixNon(marche.getPrixNon())
                .nbParticipantsOui(nbOui)
                .nbParticipantsNon(nbNon)
                .nbParticipantsTotal(nbOui + nbNon)
                .build();
    }
}